...
router.post('/api/generate-project', async (req, res) => {
  try {
    ...
    // llmResp should be a string containing JSON - parse safely
    let parsed;
    try { 
      parsed = JSON.parse(llmResp); 
    } catch (e) {
      console.warn('Direct JSON parse failed, attempting extraction:', e.message);
      // Try to extract JSON block if model added commentary
    ...
    console.error('Error in /api/generate-project:');
...
router.post('/api/generate-file', async (req, res) => {
  try {
    ...
    if (!file) return res.status(404).json({ error: 'file path not found in manifest' });
    
    file.content = content;
    await project.save();
    res.json({ path, content });
  } catch (err) {
    console.error('Error in /api/generate-file:');
    console.error('Error message:', err.message);
    console.error('Error stack:', err.stack);
    res.status(500).json({ 
      error: err.message,
...
router.post('/api/generate-all', async (req, res) => {
  try {
...
router.get('/api/download/:projectId', async (req, res) => {
  try {
